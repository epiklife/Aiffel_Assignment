...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=2cm, auto]
        % Nodes
        \node [rectangle, draw] (conv) {Convolutional\\Feature Extractor};
        \node [rectangle, draw, below of=conv] (rnn) {Recurrent\\Sequence Model};
        \node [rectangle, draw, below of=rnn] (trans) {Transcription\\Layer};
        \node [left=of conv] (input) {Input Image};
        \node [right=of trans] (output) {Text Prediction};
        ...
        \draw [->] (trans) -- (output);
    \end{tikzpicture}
    \caption{CRNN OCR System Architecture}
